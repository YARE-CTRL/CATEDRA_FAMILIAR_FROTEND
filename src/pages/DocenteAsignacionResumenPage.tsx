...
export default function DocenteAsignacionResumenPage(){
  ...
  const [textoFullOpen, setTextoFullOpen] = useState<boolean>(false);
  ...
  // Resolver href seguro para abrir en nueva pestaña
  const resolveHref = (url?: string) => {
    if (!url) return null as string | null;
    const s = String(url).trim();
    if (!s) return null;
    if (/^https?:\/\//i.test(s)) return s; // absoluto
    if (s.startsWith('/')) return s; // relativo desde raíz del sitio
    if (/^(uploads|files|storage)\//i.test(s)) return `/${s}`; // rutas comunes de archivos
    // Si no parece un archivo/ruta válida, no retornamos href para evitar navegación SPA a rutas inexistentes
    return null;
  };
  ...
  const copyRowLinks = async (entrega: any) => {
    ...
    const hrefs = items.map((it:any)=> resolveHref(it.url)).filter(Boolean) as string[];
    const txt = hrefs.join('\n');
    if (!txt) { alert('No hay enlaces válidos para copiar.'); return; }
    try { await navigator.clipboard.writeText(txt); } catch {}
  };
  const openAllLinks = (entrega: any) => {
    const items = Array.isArray(entrega?.archivos) ? entrega.archivos : [];
    const hrefs = items.map((it:any)=> resolveHref(it.url)).filter(Boolean) as string[];
    if (hrefs.length === 0) { alert('No hay enlaces válidos para abrir.'); return; }
    hrefs.forEach((h: string) => window.open(h, '_blank', 'noopener'));
  };
      ...
      <div className="max-w-6xl mx-auto space-y-6">
        ...
        {loading ? (
          ...
          <>
                      ...
                      {evidenceModal.entrega.evidenciaTexto && (
                        <div>
                          ...
                          <div className="mt-1 flex items-center gap-3">
                            <button className="text-xs text-teal-700 underline" onClick={()=> setExpandTexto(!expandTexto)}>{expandTexto ? 'Ver menos' : 'Ver más'}</button>
                            <button className="text-xs text-slate-700 underline" onClick={()=> setTextoFullOpen(true)}>Abrir grande</button>
                            <button className="text-xs text-slate-700 underline" onClick={async ()=> { try { await navigator.clipboard.writeText(String(evidenceModal.entrega.evidenciaTexto)); } catch {} }}>Copiar texto</button>
                          </div>
                        </div>
                      )}
                    </div>
                          ...
                          {evidenceModal.entrega.archivos.map((a: any, idx: number) => {
                            ...
                            return (
                              <div key={idx} className="flex items-center justify-between gap-3 px-3 py-2">
                                ...
                                <div className="flex items-center gap-2">
                                  {href ? (
                                    <a className="text-teal-700 underline text-xs" href={href} target="_blank" rel="noopener noreferrer">Abrir</a>
                                  ) : (
                                    <span className="text-slate-400 text-xs" title="Enlace inválido">Abrir</span>
                                  )}
                                  <button className="text-slate-600 text-xs underline" onClick={async ()=> { const u = resolveHref(a.url); if(!u){ alert('Enlace inválido.'); return; } try { await navigator.clipboard.writeText(u); } catch {} }}>Copiar</button>
                                </div>
                              </div>
                            );
                          })}
            ...
            <Modal
              isOpen={textoFullOpen}
              onClose={()=> setTextoFullOpen(false)}
              title="Texto de la evidencia"
              size="xl"
            >
              <div className="space-y-3">
                <div className="text-sm text-slate-600">Vista ampliada</div>
                <div className="rounded-xl border bg-white p-4 max-h-[70vh] overflow-auto">
                  <div className="whitespace-pre-wrap text-base md:text-lg leading-relaxed text-slate-800">
                    {evidenceModal.entrega?.evidenciaTexto || ''}
                  </div>
                </div>
                <div className="flex items-center justify-end gap-2">
                  <button className="px-3 py-1.5 rounded-lg border bg-white text-slate-700 text-xs" onClick={async ()=> { const t = String(evidenceModal.entrega?.evidenciaTexto || ''); if(!t){ return; } try{ await navigator.clipboard.writeText(t); } catch{} }}>Copiar</button>
                  <Button onClick={()=> setTextoFullOpen(false)} size="sm">Cerrar</Button>
                </div>
              </div>
            </Modal>
          </>
